...
class BoardView(CreateView):
    ...
    def form_valid(self, form):
        ...
        comment.photo_post = post  # Postモデルのidが「post_pk」のもの＝2行目の部分
        comment.save()
        return redirect('Photo:board', pk=post_pk)
    ...
    def get_context_data(self, **kwargs):
        ...
        post = get_object_or_404(PhotoPost, pk=post_pk)
        context['post'] = post
        context['comments'] = Comment.objects.filter(photo_post=post).order_by('-created_at')
        return context
